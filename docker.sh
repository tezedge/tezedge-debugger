...
until curl -X GET --output /dev/null --silent --head --fail "localhost:$PROXY_RPC_PORT/rpc/0/0"; do
  sleep 1
done
...
docker exec "$NODE_ID" ./tezos-node run --cors-header='content-type' --cors-origin='*' --rpc-addr 0.0.0.0:8732
